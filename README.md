...
## 重要提示！
- 因为DeepSeek的大幅涨价，连带opencode go的可用额度也大幅减少，在找到合适的替代之前DeepSeek的性价比已经降低到一个很低的水准，并且本人已不打算继续大规模使用DeepSeek，因此本项目开发将进入缓慢阶段，将在完成一些最后的收尾工作后暂时停止继续更新，如有bug可通过issue反馈，看到后会酌情考虑修复。
